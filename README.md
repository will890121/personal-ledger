...
- **平分**：`一半`、`各半`、`平分`、`N 個人平分`（人數含本人，`一半` 等同兩人）。每人負擔 = 總額 ÷ 人數；除不盡時不會四捨五入決定，而是追問每人負擔金額，追問訊息會明確寫出總額與人數。
- **明確金額**：`X 欠 <金額>`（也接受「要還」「該給」），用來表達不等額分帳。金額必須與總額寫在同一段落——一則訊息以逗號、頓號或換行分隔多筆交易時，含金額的片段會被視為獨立的一筆交易，`午餐 1260，小明欠 630` 這種逗號寫法會把「小明欠 630」切成另一筆交易而遺失分帳語意；請改寫成 `午餐 1260 小明欠 630`（用空格銜接）或在同一段落內完整表達。
