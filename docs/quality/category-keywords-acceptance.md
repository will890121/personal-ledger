...
於真實資料的**複本**上執行，結束後刪除複本。

| # | 輸入 | 預期 | 結果 |
|---|---|---|---|
| 1 | `晚餐 150` | 直接出預覽，`餐飲／晚餐`，資金流出 | ☐ |
| 2 | `早餐 100 現金` | 直接出預覽，`餐飲／早餐`，資金流出（帳戶不再讓它變成午餐） | ☐ |
| 3 | `電影 300 國泰卡` | 直接出預覽，`娛樂／電影`，不影響當下可動用資金 | ☐ |
| 4 | `捷運 30` | 直接出預覽，`交通／捷運` | ☐ |
| 5 | `Uber 245` | 直接出預覽，`交通`（無品項）、商家：Uber | ☐ |
| 6 | `國泰卡刷 1200` | **追問分類**，候選按鈕出現「餐飲」而非「午餐」 | ☐ |
| 7 | `一蘭拉麵 200` | **追問分類**（店名刻意不進表，見 `docs/todo/merchant-memory.md`） | ☐ |
| 8 | `午餐 1260，小明欠 630` | 代墊路徑不受影響，兩筆配置皆為 `餐飲／午餐` | ☐ |
| 9 | `/recent` | 舊資料仍顯示原本的 `餐飲／午餐`，快照未被改寫 | ☐ |
| 10 | 重啟服務 | 啟動不報錯（bootstrap 不會撞 UNIQUE），schema 為 6 | ☐ |
